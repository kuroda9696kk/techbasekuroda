...
<body>
	...
	$editCheck = $_POST['numEdit1']; //編集番号を受け取る
	//echo "editCheck".$editCheck."<br/>";
	
	$password = $_POST['password'];
	if($password == "abehiroshi" || $editCheck !== 'なし'){
		changeComment($name,$comment,$filename,$editCheck);
	}else{
		echo "パスワードが違います";
	}
	
	
	function changeComment($name,$comment,$filename,$editCheck){
	//echo $editCheck."<br/>";
    /*
    普通にコメントを投稿した時
    */
    if($editCheck== 'なし' &&!empty($_POST['name'])&&!empty($_POST['comment'])&&empty($_POST['numRemove'])){ //名前とコメントが両方入っててかつ削除欄が空の時
      /*
      送信した投稿とコメントをブラウザに表示
      */
     ...
     */ 
     ...
     /*
      削除行の投稿とコメントをブラウザに表示
      */
      echo "コメント".$numRemove."を"."削除しました。　".$removedDate . '<br/>';
